...
const sheetVariants = cva(
  "bg-card fixed z-50 flex flex-col gap-4 shadow-xl transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-250 data-[state=open]:duration-300",
  {
    variants: {
      side: {
  ...
  return (
    <SheetPortal>
      ...
      >
        {children}
        {showClose ? (
          <DialogPrimitive.Close className="text-muted-foreground hover:bg-accent absolute top-4 right-4 rounded-md p-1.5 opacity-70 transition hover:opacity-100 focus-visible:outline-none">
            <X className="size-4" />
