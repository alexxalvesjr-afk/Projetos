...
function normalisePayload(input: z.infer<typeof vehicleSchema>) {
  return {
    ...
    steering: input.steering ? normaliseText(input.steering) : null,
    traction: input.traction ? normaliseText(input.traction) : null,
    horsepower: input.horsepower ?? null,
    seats: input.seats ?? null,
    valvesPerCylinder: input.valvesPerCylinder ?? null,
    fuelTankLiters: input.fuelTankLiters ?? null,
